...
public class OrderService {
    ...
    public OrderResponse placeOrder(PlaceOrderRequest orderRequest) {
        ...
        OrderEntity order = OrderEntity.builder().orderId(username.substring(0, 3).toUpperCase() + "_" + UUID.randomUUID()).username(username).emailId(user.getEmail()).giftType(GiftTypeEnum.valueOf(orderRequest.getGiftType())).amountPaid(orderRequest.getAmountPaid()).orderStatus(OrderStatusEnum.ORDER_RECEIVED).build();
